        ...
        if ($this->getType() == 'info') {
            ...
            if ($eqLogic->getConfiguration('type') == 'yeelight') {
                ...
                if ($this->getLogicalId() == 'refresh') {
                    ...
                    $value = json_encode(array('apikey' => jeedom::getApiKey('xiaomihome'), 'type' => 'aquara','cmd' => 'read', 'model' => $eqLogic->getConfiguration('model'),  'dest' => $gateway , 'password' => $password,'sidG' => $xiaomihome->getConfiguration('sid'), 'sid' => $eqLogic->getConfiguration('sid')));
                    xiaomihome::sendDaemon($value);
                    return;
                }
